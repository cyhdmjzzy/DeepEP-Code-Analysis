...
// 线程块集群（Thread Block Clusters）参见：https://osvicyu5w5.feishu.cn/wiki/BJx1wy9Xzi2P8skqF8tcmHJ8nOf
/*

// attr[0]用于设置启用协作组内核（Cooperative Groups）的相关配置。
attr[0].id = cudaLaunchAttributeCooperative;

// 设置为协作模式，允许同一个cluster中的block间的同步。这是使用cluster功能的必要条件。
attr[0].val.cooperative = 1;

// attr[1]用于设置指定线程块簇的维度
attr[1].id = cudaLaunchAttributeClusterDimension;

// 设置cluster的 x 方向上的block数量
