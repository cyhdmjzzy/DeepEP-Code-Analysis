...
import deep_ep_cpp   # 导入编译好的C++扩展（deep_ep_cpp.so文件）
# noinspection PyUnresolvedReferences
...
class Buffer:
    ...
    def __init__(self,
        ...
        Arguments:
            group: the communication group.  通信组。
            num_nvl_bytes: the buffer size for intranode NVLink communication.  节点内 NVLink 通信的缓冲区大小（字节）。
            num_rdma_bytes: the buffer size for internode (also for intranode with low-latency mode) RDMA communication.
                            节点间（以及在低延迟模式下用于节点内）RDMA 通信的缓冲区大小（字节）。
            low_latency_mode: whether to enable low-latency mode.  是否启用低延迟模式。
            num_qps_per_rank: the number of QPs for RDMA, the low-latency mode requires that this number equals
                to the number of local experts.
                每个 rank 用于 RDMA 的 QP 数量。低延迟模式要求此数量等于本地 experts 的数量。
            allow_nvlink_for_low_latency_mode: whether allow NVLink traffic for low-latency mode, you should notice
                ...
                是否允许在低延迟模式下使用 NVLink 通信。请注意，这在某种程度上与基于 hook 的重叠（overlapping）机制不兼容。
                警告: PCIe 连接可能会因内存排序问题导致错误，请确保所有连接都通过 NVLink。
            allow_mnnvl: whether to allow MNNVL.  是否允许 MNNVL（可能是指 Multi-Node NVLink，一种多节点 NVLink 技术或相关优化）。
            use_fabric: whether to use fabric API for memory buffers.  是否使用 fabric API 来管理内存缓冲区。
            enable_shrink: whether to enable shrink mode. The enable mode allocates a mask buffer to support masking ranks dynamically.
                           是否启用收缩（shrink）模式。启用此模式会分配一个掩码（mask）缓冲区，以支持动态地屏蔽（masking）某些 rank。
            explicitly_destroy: If this flag is set to True, you need to explicitly call `destroy()` to release resources;
                otherwise, the resources will be released by the destructor.
                Note: Releasing resources in the destructor may cause Python's exception handling process to hang.
                如果此标志设置为 True，你需要显式调用 destroy() 方法来释放资源；否则，资源将在析构函数中释放。
                注意: 在析构函数中释放资源可能会导致 Python 的异常处理过程挂起。
            comm: the `mpi4py.MPI.Comm` communicator to use in case the group parameter is absent.
                  当 group 参数不存在时，使用的 mpi4py.MPI.Comm 通信器。
        """
    ...
    def capture() -> EventOverlap:
        ...
        """
        # 在当前流（current_stream）中捕获一个CUDA事件，创建的 EventHandle 对象record在当前stream中。
    ...
    def dispatch(self, x: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
        ...
        Arguments:
            ...
            previous_event: the event to wait before actually executing the kernel.
                            在实际执行dispatch核函数之前，需要等待previous_event完成。
            async_finish: the current stream will not wait for the communication kernels to be finished if set.
            allocate_on_comm_stream: control whether all the allocated tensors' ownership to be on the communication stream.
        ...
        if handle is not None:
            ...
            recv_x, recv_x_scales, recv_topk_idx, recv_topk_weights, num_recv_tokens_per_expert_list, rank_prefix_matrix, channel_prefix_matrix, \
    ...
    def internode_dispatch(self, x: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
        ...
        if handle is not None:
            ...
            """
            对比 intranode_dispatch 的调用
            recv_x, recv_x_scales, recv_topk_idx, recv_topk_weights, num_recv_tokens_per_expert_list, rank_prefix_matrix, channel_prefix_matrix, \
            recv_channel_prefix_matrix, recv_src_idx, send_head, event = self.runtime.intranode_dispatch(
    ...
    def low_latency_dispatch(self, x: torch.Tensor, topk_idx: torch.Tensor,
        ...
        Arguments:
            ...
            TODO round_scale 控制 FP8 量化中缩放因子的计算方式，True时将因子舍入为2的幂次方，支持更高效的UE8M0格式存储。
    ...
    def low_latency_combine(self, x: torch.Tensor, topk_idx: torch.Tensor, topk_weights: torch.Tensor,
        ...
        Arguments:
            ...
            combine_wait_recv_cost_stats: a cumulative time spent waiting to receive each token tensor for statistics,
                which should have shape `[num_ranks, num_ranks]` and be typed as `torch.int64`.  shape应该是[num_ranks]
