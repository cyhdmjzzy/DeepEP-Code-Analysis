...
def test_main(num_tokens: int,
                ...
                for round_scale in (False, True) if dispatch_use_fp8 else (False, ):
                    for use_ue8m0 in (False, True) if round_scale else (False, ):
                        ...
                        for zero_copy in (False, ) if use_logfmt else (False, True):
                            if zero_copy:
                                # PyTorch 触发 GPU 到 GPU 的内存拷贝。数据从普通 GPU 内存拷贝到 RDMA 注册内存。
                                # simulated_gemm_x 相当于是本地所有专家的输出tensor的组合，现在要写入到RDMA缓冲区中。
                                # zero_copy 正是为了减少 expert 输出到 combine 发送缓冲区的拷贝过程，而不用先把专家输出拷贝到GPU普通内存，然后从GPU普通内存拷贝到本地的RDMA发送缓冲区。
    ...
    def large_gemm_with_hook(hook):
        ...
        """
        DeepEP只提供通信原语（dispatch/combine）和Hook机制。
        如果你想在通信的同时进行计算，你需要自己实现计算逻辑。
        这是一个人工构造的计算任务，用来模拟在通信传输期间可以并行执行的计算。
        mat_0 @ mat_1是一个大矩阵乘法，耗时较长
        测试的目的：
        1、验证Hook机制是否能正确实现通信-计算重叠
        2、测量使用Hook和不使用Hook的性能差异
        3、证明通信传输期间可以执行其他计算
        """
        mat_0 @ mat_1
        hook()
